...
MODEL_URL = "https://koamodel.s3.ap-southeast-2.amazonaws.com/OSTEO_VISION_MODEL_6_+Knee+Osteoarthritis+Detection+with+Fine-Tuned+ResNet152V2+on+dataset+3+MODEL_6.h5"

MODEL_DIR = os.path.join("src", "models")
MODEL_PATH = os.path.join(MODEL_DIR, "OSTEO_VISION_MODEL_6.h5")
...
def download_model_from_url(url, output_path):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to download model. HTTP Status code: {response.status_code}"
        )
...
if not os.path.exists(MODEL_PATH):
    st.info("Downloading AI model from AWS S3. First launch may take a few minutes...")
